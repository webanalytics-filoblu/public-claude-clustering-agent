...
description: "Genera un CSV di regole Cluster/Sottocluster/Terms dedicate a un brand, nello stesso formato dei file 'Regole' usati da seo-keyword-clustering, a partire da: URL della piattaforma e-commerce del brand, URL delle voci di menu estratte dalla piattaforma, link alle linee guida/tone of voice del brand, e un CSV di keyword del brand già assegnate a un cluster/tema generico. Usa SEMPRE questa skill quando l'utente chiede di creare, generare, estendere o integrare un file di regole di clustering specifico per un brand (fashion/shoes/intimo/multibrand), o parla di 'regole cluster per il brand X', 'sottocluster dedicati al brand', o fornisce menu + keyword di un brand e chiede di produrre un csv di regole. Non usare per la clusterizzazione delle keyword stesse (quella è seo-keyword-clustering) né per la pulizia di export Semrush grezzi (semrush-keyword-cleaner)."
---
